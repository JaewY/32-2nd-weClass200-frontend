...
import { TAB_MENU } from "../ProductDetailData";
...
const ProductDetailMain = ({ detail, setDetail }) => {
  ...
  //   fetch("http://10.58.7.33:8000/users/review", {
  //     method: "DELETE",
  //     body: JSON.stringify({
  //       user_id: "9",
  //       product_id: "1",
  ...
  return (
    <LeftSide>
      ...
      <TabMenus>
        {TAB_MENU.map(menu => {
          return (
            <Link key={menu.id} to={menu.id} spy={true} smooth={true}>
              <TabMenu onClick={handleTabClick} ref={tabMenu}>
                {/* <TabMenu onClick={handleTabClick}  ref={tabMenu} color fontWeight border> */}
                {menu.title}
              </TabMenu>
            </Link>
          );
      ...
      <ClassInfo>
        ...
        <ClassInfoText>
          <Text margin="12px 0 0 0" color="rgb(162, 162, 162)">
            클래스 분량
          </Text>
          <Text margin="12px 48px 0 0">14개 챕터, 66개 세부강의</Text>
          <Text margin="12px 0 0 0" color="rgb(162, 162, 162)">
            수강 가능일
          </Text>
          <Text margin="12px 48px 0 0">바로 수강 가능</Text>
          <Text margin="12px 0 0 0" color="rgb(162, 162, 162)">
            자막 포함 여부
          </Text>
          <Text margin="12px 48px 0 0">포함</Text>
        </ClassInfoText>
      </ClassInfo>
        ...
        <ContentBox>
          ...
          <Dialog open={openModal}>
            ...
            <ImgPreview
              ...
              alt="img"
            />
            {/* <ImgInputLabel for="image">여기서 파일 선택하세용</ImgInputLabel> */}
            <ReviewImgInput
              id="image"
            ...
            {detail[0]?.reviews.map(review => {
              return (
                <Review key={review.id}>
                  ...
                  {/* <ReviewDelete onClick={handleDeleteClick}>X</ReviewDelete> */}
                </Review>
              );
            })}
          </ReviewBox>
